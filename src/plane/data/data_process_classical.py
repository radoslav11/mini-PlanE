...
class ENCODING:
    ...
    def get_cycles(self):
        ...
        if self.spqr_node is not None:
            ans[self.spqr_node] = (self.spqr_cycle, self.spqr_code)
...
def construct_spqr_code(spqr_node, cycle, nid, node_code, edge_code):
    ...
    code.spqr_code = [nid[u] for u, v, label in cycle]
    return code


def encode_tcc_with_edge_with_direction(
    spqr_node: SPQR_NODE,
    e0: CUT_PAIR,
    node_code: NODE_MAP,
    edge_code: EDGE_MAP,
    ...
    if not hasattr(g, "_embedding"):
        ...
        g._embedding_index = {
            u: {v: i for i, v in enumerate(neigh)}
...
def encode_r_node(
    ...
    """
    return min(
        encode_tcc_with_edge_with_direction(
            spqr_node, e0, node_code, edge_code, d
        )
        for d in [-1, 1]
    )
...
def encode_r_root(
    spqr_node: SPQR_NODE, node_code: NODE_MAP, edge_code: EDGE_MAP
):
...
def encode_bcc(
    bcc_sage: sageall.Graph, node_code0: NODE_MAP, edge_code0: EDGE_MAP
):
    ...
    min_code, min_center = min(
        ((encode_from_center(center), center) for center in tree.center()),
        key=lambda x: x[0],
    )
...
def encode_cc(
    cc_sage: sageall.Graph, node_code: NODE_MAP, edge_code: EDGE_MAP
):
        ...
        for fa, cur in reversed(
            ...
            match type_cur:
                case "B":
                    ...
                    bcc_codes[node_cur] = encode_bcc(
                        cc_sage.subgraph(node_cur), node_code, edge_code
                    )
                case "C":
                    code = ENCODING()
